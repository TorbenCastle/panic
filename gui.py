...
class GuiHandler:
    ...
    def show_popup(self, popup_id, client = None):
        if popup_id not in self.popup_statuses or not self.popup_statuses[popup_id]:
            # Open the popup if it's not opened
        ...
        # Only open a new popup if one is not already open
            if(popup_id == "Clear"):
                ...
                self.chat_window = ShowDataPopup(self.root, self, popup_id, "Chat", "200x120", "chat")
...
#update the status of the buttons
    ...
    def create_chat_message(self, sender, message):
        ...
        # Update chat log if the chat window is open, or open it to see new messages
        if self.popup_statuses.get("Chat", 0):
...
class ShowDataPopup(PopupWindow):
    def __init__(self, parent, gui_handler, popup_id , name=None, size=None,  arg1=None, arg2=None, arg3=None , arg4=None):
        ...
        if arg1 == "chat":
            ...
            messages = []
            for entry in self.gui_handler.chat_messages:
                messages.append(entry)

            def update_chat():
                self.chat_text.configure(state=tk.NORMAL)
                self.chat_text.insert(tk.END, self.gui_handler.chat_messages[-1] + "\n")
